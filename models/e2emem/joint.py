...
train, test = [], []
for i in ids:
    tr, te = load_task(FLAGS.data_dir, i, only_supporting=True)
    train.append(tr)
    test.append(te)
...
np.save('lookupvocab', lookup_vocab)
        ...
        if i % FLAGS.evaluation_interval == 0:
            ...
            for task in tasks:
                ...
                if acc > best_val_accs[task]:
                    ...
                    s = testS[start:end]
                    q = testQ[start:end]
                    #pred, probs_hops, pred_proba = model.predict_proba(s, q)
                    A, B, C, TA, TC, H, W, pred, probs_hops, pred_proba = model.predict_wlogs(s, q)
                    pred_word = [reverse_lookup[pred_iter] for pred_iter in pred]
                    true_word = [reverse_lookup[label_iter] for label_iter in test_labels]
                    # print(word_idx)
                    # print(reverse_lookup)
                    np.save('attention_task'+str(task), probs_hops)
                    np.save('predprob_task'+str(task), pred_proba)
                    np.save('pred_task'+str(task), pred_word)
                    np.save('truth_task'+str(task), true_word)
                    np.save('A_task'+str(task), A)
                    np.save('B_task'+str(task), B)
                    np.save('C_task'+str(task), C)
                    np.save('TA_task'+str(task), TA)
                    np.save('TC_task'+str(task), TC)
                    np.save('H_task'+str(task), H)
                    np.save('W_task'+str(task), W)
