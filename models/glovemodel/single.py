...
from data_utils import load_task, vectorize_data, read_word_embeddings
...
S, Q, A = vectorize_data(train, word_vectors, word_idx, sentence_size, memory_size)
trainS, valS, trainQ, valQ, trainA, valA = cross_validation.train_test_split(S, Q, A, test_size=.1, random_state=FLAGS.random_state)
testS, testQ, testA = vectorize_data(test, word_vectors, word_idx, sentence_size, memory_size)
...
with tf.Session() as sess:
    model = MemN2N(batch_size, vocab_size, sentence_size, memory_size, FLAGS.embedding_size, word_vectors, session=sess,
